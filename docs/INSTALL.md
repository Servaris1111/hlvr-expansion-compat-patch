...
Do not launch Opposing Force or Blue Shift VR through Steam's `Change Game` menu. These batch files are required because they start HLVR with the correct expansion folder, client DLL, first map, and expansion-safe audio settings before HLVR initializes sound.
...
- Uses world/player weapon models for controller-held weapons so first-person `v_*.mdl` arm meshes are not drawn as broken floating arms.
...
- Repairs root `hlvr.cfg` if it is missing or truncated, restoring default HLVR hand, wrist HUD, movement, and weapon settings.
- Keeps HLVR client commands out of expansion `autoexec.cfg` aliases; the proxy DLL handles command filtering.
